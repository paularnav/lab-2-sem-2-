...
//declaring class for LinkedList
class LinkedList{
private:
    Node *head;
    Node *tail;
    int n; //initializing size of LinkedList
public:
...
//Insertion of node at given position
    void InsertAt(int x, int y){//x=data and y=position
        if (y>n+1){
            ...
            Node *locate;  //for finding the position where the node is to be inserted
            locate=head;
            for (int i=1;i<y-1;i++){
                locate=locate->next;
            }
...
//deletion of an element from the end
    void Delete()
    {
        Node *locate;  //for locating the last node
        locate=head;
        ...
        tail=locate;
        locate->next=NULL;
        --n;//decrement in no of nodes of LinkedList
    }
//deletion of an element at a given position
    void DeleteAt(int x){
        if (x>n)
            ...
            Node *temp; //temporary node which is to be deleted later
            temp=head;
            head=temp->next;
            delete temp;
        }
        else {
            Node *locate; //for locating the position of the node to be deleted
            locate=head;
            for (int i=1;i<x-1;i++){
                locate=locate->next;
            }
...
//for counting the number of elements in the LinkedList
    int Count(){
    return n;}
//for displaying the elements of the LinkedList
    void Display(){
        Node *disp;
        disp=head;
        while (disp!=NULL){
